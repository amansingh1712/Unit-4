...
const RestaurantDetails = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/restaurant?_page=${pageNumber}&_limit=5`)
      .then((res) => res.json())
  ...
  const handleRateSort = (query) => {
    fetch("http://localhost:8080/restaurant")
      .then((res) => res.json())
      .then((data) => {
        let temp = [];
        if (query === "htl") {
          temp = cardData.sort((a, b) => b.stars - a.stars);
        } else {
          temp = cardData.sort((a, b) => a.stars - b.stars);
        }
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <div>
            <p>Rating sort</p>
            <button onClick={() => handleRateSort("htl")}>
              Sort high to low
            </button>
            <button onClick={() => handleRateSort("lth")}>
              Sort low to high
            </button>
          </div>
          <div>
      ...
      <h3 style={{ textAlign: "center" }}>Page: {pageNumber}</h3>
